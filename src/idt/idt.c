...
static INTERRUPT_CALLBACK_FUNCTION interrupt_callbacks[HARDIKHYPERIONOS_TOTAL_INTERRUPTS];
...
{
    if (interrupt < 0 || interrupt >= HARDIKHYPERIONOS_TOTAL_INTERRUPTS)
    {
        return -EINVARG;
    }
...
void *isr80h_handle_command(int command, struct interrupt_frame *frame)
{
    ...
    if (command < 0 || command >= HARDIKHYPERIONOS_MAX_ISR80H_COMMANDS)
    {
        // Invalid command
        return 0;
    }
