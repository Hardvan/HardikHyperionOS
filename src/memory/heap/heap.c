...
static int heap_validate_table(void *ptr, void *end, struct heap_table *table)
{
    ...
    size_t total_blocks = table_size / HARDIKHYPERIONOS_HEAP_BLOCK_SIZE;
...
{
    return ((unsigned int)ptr % HARDIKHYPERIONOS_HEAP_BLOCK_SIZE == 0);
}
...
{
    if (val % HARDIKHYPERIONOS_HEAP_BLOCK_SIZE == 0)
    {
        return val;
    }

    val = (val - (val % HARDIKHYPERIONOS_HEAP_BLOCK_SIZE)); // Align to lower
    val += HARDIKHYPERIONOS_HEAP_BLOCK_SIZE;                // Align to upper
...
{
    return heap->saddr + (block * HARDIKHYPERIONOS_HEAP_BLOCK_SIZE);
}
...
{
    return ((int)(address - heap->saddr)) / HARDIKHYPERIONOS_HEAP_BLOCK_SIZE;
}

void *heap_malloc(struct heap *heap, size_t size)
{
    size_t aligned_size = heap_align_value_to_upper(size);
    uint32_t total_blocks = aligned_size / HARDIKHYPERIONOS_HEAP_BLOCK_SIZE;
