...
#define HARDIKHYPERIONOS_FAT16_SIGNATURE 0x29
#define HARDIKHYPERIONOS_FAT16_FAT_ENTRY_SIZE 0X02
#define HARDIKHYPERIONOS_FAT16_BAD_SECTOR 0XFF7
#define HARDIKHYPERIONOS_FAT16_UNUSED 0X00
...
struct fat_private
{
    ...
    // Used to stream data from the fat table
...
int fat16_get_total_items_for_directory(struct disk *disk, uint32_t directory_start_sector)
{
    ...
    if (diskstreamer_seek(stream, directory_start_pos) != HARDIKHYPERIONOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    while (1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != HARDIKHYPERIONOS_ALL_OK)
        {
            res = -EIO;
            goto out;
        }

        // If we hit an empty item, we are done
        if (item.filename[0] == 0x00)
        {
            break;
        }

        // If we hit a deleted item, continue
                             ...
                             struct fat_directory *directory)
{
    ...
    struct disk_stream *stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != HARDIKHYPERIONOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != HARDIKHYPERIONOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_resolve(struct disk *disk)
{
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != HARDIKHYPERIONOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    if (fat_private->header.shared.extended_header.signature != HARDIKHYPERIONOS_FAT16_SIGNATURE)
    {
        res = -EFSNOTUS;
        goto out;
    }

    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != HARDIKHYPERIONOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
static int fat16_get_fat_entry(struct disk *disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * HARDIKHYPERIONOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk *disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        if (entry == HARDIKHYPERIONOS_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
static int fat16_read_internal(struct disk *disk,
                               int starting_cluster, int offset, int total, void *out)
{
...
void fat16_fat_item_free(struct fat_item *item)
{
    ...
    kfree(item); // Free the item itself
}
                                              ...
                                              const char *filename)
{
    ...
    char tmp_filename[HARDIKHYPERIONOS_MAX_PATH];
